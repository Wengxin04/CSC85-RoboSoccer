...
struct AI_data {
  // This data structure is used to hold all data relevant to the state of the
  // AI. This includes, of course, the current state, as well as the status of
 ...
 * \param[out] void, but the state description in the AI structure may have
 * changed
...
// the AI data structure provides a way for you to add graphical markers
// on-screen, the functions below add points or lines at a specified location
// and with the colour you want. Items you add will remain there until cleared.
// Do not mess with the list directly, use the functions below! Colours are
// specified as floating point values in [0,255], black is [0,0,0] white is
// [255,255,255].
...
void rotate_to_blob(struct RoboAI *ai, double smx, double smy, double target_x,
                    double target_y);
void move_to_blob(struct RoboAI *ai, double smx, double smy, double t_cx,
                  double t_cy, double target_dist);
