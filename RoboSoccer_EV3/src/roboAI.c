...
static bool is_facing_target(struct RoboAI *ai, double smx, double smy,
...
static bool is_close_to_ball(struct RoboAI *ai, double ball_cx,
                             double ball_cy) {
  double de = 0, dd = 0;
  double d =
...
static bool is_close_to_target(struct RoboAI *ai, double target_cx,
                               double target_cy) {
  if (!ai || !ai->st.self)
    return false;
...
static int check_ball_position(struct RoboAI *ai) {
  ...
  // ??? Need to reconsider when to defend, right now just consider when ball is
  ...
  // random values for now: 200 pixels from edge for y, 400 pixels from edge for
  // x
...
static void penalty_mode(struct RoboAI *ai, double *stored_smx,
                         double *stored_smy) {
  ...
  case ST_PENALTY_ROTATE_TO_TARGET:
    ...
    // break;
    {
      ...
      if (is_facing_target(ai, *stored_smx, *stored_smy, target_cx,
                           target_cy) &&
          rotate_flag == -1) {
...
// 上下左右 0123
static int compute_ball_nearest_edge(struct RoboAI *ai, double *edge_x,
                                     double *edge_y) {
  if (!ai || !ai->st.ball)
    return -1;
